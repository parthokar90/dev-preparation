    ...
    return (
        <div className="flex min-h-screen bg-white text-black">
            ...
            <aside className="w-64 border-r-2 border-black p-6 flex flex-col justify-between">
                <div>
                    <nav className="space-y-2">
                        <button
                            ...
                            className={`w-full text-left p-3 font-bold text-sm uppercase border-2 transition ${activeMenu === "categories" ? "bg-black text-white border-black" : "border-transparent hover:border-black"
                                }`}
                        >
                        ...
                        <button
                            ...
                            className={`w-full text-left p-3 font-bold text-sm uppercase border-2 transition ${activeMenu === "topics" ? "bg-black text-white border-black" : "border-transparent hover:border-black"
                                }`}
                        >
                ...
                <div className="text-xs font-mono text-neutral-400">v1.1.0 · Secured Session</div>
            </aside>
            ...
            <main className="flex-1 p-10">
                    ...
                    <div>
                        <div className="flex justify-between items-center mb-8">
                            ...
                            <button
                                ...
                                className="bg-black text-white px-5 py-2.5 text-sm font-bold uppercase border-2 border-black hover:bg-white hover:text-black transition"
                            >
                                + Add Category
                            </button>
                        </div>
                        ...
                        <div className="border-2 border-black overflow-hidden">
                            <table className="w-full text-left border-collapse">
                                <thead className="bg-neutral-50 border-b-2 border-black font-mono text-xs uppercase tracking-wider">
                                    <tr>
                                        <th className="p-4 font-bold border-r-2 border-black w-16">ID</th>
                                        <th className="p-4 font-bold border-r-2 border-black">Category</th>
                                        <th className="p-4 font-bold border-r-2 border-black">Description</th>
                                        <th className="p-4 font-bold w-40 text-center">Actions</th>
                                    </tr>
                                </thead>
                                    ...
                                    {loading ? (
                                        ...
                                        categories.map((cat) => (
                                            <tr key={cat.id} className="hover:bg-neutral-50/50 transition">
                                                <td className="p-4 border-r-2 border-black font-mono font-bold bg-neutral-50/20">{cat.id}</td>
                                                <td className="p-4 border-r-2 border-black font-bold uppercase tracking-tight">{cat.name}</td>
                                                <td className="p-4 border-r-2 border-black text-neutral-600 max-w-xs truncate">{cat.description || "—"}</td>
                                                <td className="p-4 text-center space-x-4 font-mono">
                ...
                {activeMenu === "topics" && (
                    <div>
                        <div className="mb-8">
                            <h1 className="text-3xl font-black uppercase tracking-tight">Topics Registry</h1>
                        ...
                        {/* ADD/EDIT TOPIC FORM */}
                        <form onSubmit={handleAddTopic} className="mb-8 p-6 border-2 border-black bg-neutral-50 flex flex-col gap-5">

                            {editingTopic && (
                                <div className="flex justify-between items-center bg-yellow-100 border-2 border-black px-3 py-2 text-xs font-bold uppercase">
                                    <span>Editing Topic #{editingTopic.id}</span>
                                    <button type="button" onClick={handleCancelEditTopic} className="underline hover:text-red-600">Cancel</button>
                                </div>
                            )}
                        ...
                        <div className="border-2 border-black overflow-hidden">
                            <table className="w-full text-left border-collapse">
                                <thead className="bg-neutral-50 border-b-2 border-black font-mono text-xs uppercase tracking-wider">
                                    <tr>
                                        <th className="p-4 font-bold border-r-2 border-black">Category</th>
                                        <th className="p-4 font-bold border-r-2 border-black">Title</th>
                                        <th className="p-4 font-bold border-r-2 border-black">Description</th>
                                        <th className="p-4 font-bold w-32 text-center">Actions</th>
                                    </tr>
                                </thead>
                                <tbody className="divide-y-2 divide-black text-sm">
                                    {topicLoading ? (
                                        ...
                                        topics.map((topic) => {
                                            ...
                                            return (
                                                <tr key={topic.id} className="hover:bg-neutral-50 transition">
                                                    <td className="p-4 border-r-2 border-black font-mono font-bold uppercase text-xs bg-neutral-50/50">
                                                        {parentCategory ? parentCategory.name : "—"}
                                                    </td>
                                                    <td className="p-4 border-r-2 border-black font-bold tracking-tight">{topic.title}</td>
                                                    <td className="p-4 border-r-2 border-black text-neutral-600 text-xs max-w-xs truncate">{topic.description || "—"}</td>
                                                    <td className="p-4 text-center space-x-4 font-mono">
            ...
            {isModalOpen && (
                <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center p-4 z-50">
                    <div className="bg-white border-2 border-black p-6 w-full max-w-md shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
